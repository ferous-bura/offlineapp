...
import basesdk.util as util
from basesdk import exceptions
from basesdk.conf import __version__, __endpoint_map__
...
class Api(object):
    ...
    user_agent = "basesdk/SUPERAPP-Python-SDK %s (%s)" % (
    ...
    def __init__(self, options=None, **kwargs):
        ...
        Usage::
            ...
            >>> import basesdk
            >>> api = basesdk.Api(mode="sandbox", client_id='CLIENT_ID', client_secret='CLIENT_SECRET',
    ...
    def basic_auth(self):
        ...
        """
        print(f'basic auth {self.client_id}, {self.client_secret}')
    ...
    def get_token_hash(self, authorization_code=None, refresh_token=None, headers=None):
        ...
        if authorization_code is not None:
            ...
            if self.token_hash is not None:
                # return cached copy
                # print(f'returning catched token: {self.token_hash.get('access_token')}')
    ...
    def get_refresh_token(self, authorization_code=None, headers=None):
        ...
        if authorization_code is None:
            ...
            Refer to https://developer.superapp.com/docs/integration/mobile/make-future-payment/#get-an-auth-code")
    ...
    def _check_openssl_version(self):
        ...
        if self.ssl_version_info and self.ssl_version_info < (1, 0, 1, 0, 0):
            log.warning(
                ...
                (https://github.com/superapp/TLS-update), you MUST update to the latest security library.')

    def request(self, url, method, body=None, headers=None, refresh_token=None):
        """Make HTTP call, formats response and does error handling. Uses http call method in API class.

        Usage::

            >>> api.request("https://api.sandbox.superapp.com/v1/payments/payment?count=10", "GET", {})
            >>> api.request("https://api.sandbox.superapp.com/v1/payments/payment", "POST", "{}", {} )

        """

        print(f'*********** method in request is {method} ************')

        http_headers = util.merge_dict(
            self.headers(refresh_token=refresh_token, headers=headers or {}), headers or {})

        if http_headers.get('SUPERAPP-Request-Id'):
            log.info('SUPERAPP-Request-Id: %s' %
                     (http_headers['SUPERAPP-Request-Id']))
    ...
    def http_call(self, url, method, **kwargs):
        ...
        debug_id = response.headers.get('superapp-Debug-Id')
        if debug_id:
        ...
        # print(f'http call - response {response}')
    ...
    def post(self, action, params=None, headers=None, refresh_token=None):
        ...
        print(f"************  post create refresh_token {refresh_token} params {params}, endpoint {self.endpoint} headers {self.headers} *************")
...
def default():
    ...
    if __api__ is None:
        try:
            client_id = os.environ["SUPERAPP_CLIENT_ID"]
            client_secret = os.environ["SUPERAPP_SECRET_KEY"]
        except KeyError:
            raise exceptions.MissingConfig("Required PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET. \
                Refer https://github.com/superapp/rest-api-sdk-python#configuration")

        __api__ = Api(mode=os.environ.get(
            "SUPERAPP_MODE", "sandbox"), client_id=client_id, client_secret=client_secret)
    return __api__
...
def set_config(options=None, **config):
    ...
    print(f'config {config}, api {__api__}')
    print(f'config {config}, api {__api__.basic_auth}')
    print(f'config {config}, api {__api__.client_secret}')
    print(f'config {config}, api {__api__.headers}')
    return __api__
