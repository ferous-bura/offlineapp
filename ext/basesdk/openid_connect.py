...
from basesdk.resource import Resource
from basesdk.api import default as default_api
from basesdk.api import Api
from basesdk.conf import __version__
from six import string_types

class Base(Resource):

    user_agent = "SuperAppSDK/openid-connect-python %s (%s)" % (__version__, Api.library_details)
...
class Tokeninfo(Base):
    ...
    https://developer.superapp.com/help/docs/api/#identity
    """
...
def session_url(path, options=None, api=None):
    api = api or default_api()
    if api.mode == "live":
        path = util.join_url("https://www.superapp.com", path)
    else:
        path = util.join_url("https://www.superapp.paypal.com", path)
